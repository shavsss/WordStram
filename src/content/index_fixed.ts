...
function createGeminiPanel() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      thinkingElement.className = 'gemini-message assistant-message error';
      
      // אנחנו לא מוחקים את הקלט במקרה של שגיאה
      // כך שהמשתמש יוכל לנסות שוב או לערוך את השאלה
    }
    
    // גלילה אוטומטית לחלק התחתון שוב אחרי הוספת התשובה
    conversationArea.scrollTop = conversationArea.scrollHeight;
  };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // שימוש בפונקציה החדשה במקום הישנה
  setTimeout(renderReactControls, 1000);
...
window.addEventListener('load', () => {
  ...
  startDetection();
  // שימוש בפונקציה החדשה במקום הישנה
  setTimeout(renderReactControls, 2000);
});
