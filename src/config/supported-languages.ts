...
export type LanguageName = typeof LANGUAGE_MAP[LanguageCode];
export type SupportedLanguageCode = LanguageCode;

export function getLanguageCode(name: LanguageName): LanguageCode | undefined {
  const entries = Object.entries(LANGUAGE_MAP) as [LanguageCode, LanguageName][];
  const found = entries.find(([_, value]) => value === name);
  return found?.[0];
}
