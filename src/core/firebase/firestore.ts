...
let syncState = {
  initialized: false,
  unsubscribeFunctions: [] as Array<() => void>,
  pendingOperations: [] as Array<{ type: string, data: any, timestamp: number }>,
};
...
async function processPendingOperations(): Promise<void> {
  ...
  console.log(`WordStream: Processing ${syncState.pendingOperations.length} pending operations`);
  
  // Sort operations by timestamp (oldest first)
  const operations = [...syncState.pendingOperations].sort((a, b) => a.timestamp - b.timestamp);
 ...
 */
function queueOperation(type: string, data: any): void {
  syncState.pendingOperations.push({
    type,
    data,
    timestamp: Date.now()
  });
...
export async function saveNote(note: any): Promise<string> {
  ...
  try {
    // Check if online first - try to save to Firestore immediately
    if (navigator.onLine) {
      try {
    ...
    saveNoteToLocalStorage(noteWithId);
        ...
        // Add user ID and last synced timestamp
        noteWithId.userId = userId;
        noteWithId.lastSynced = new Date().toISOString();
        
        // Use consistent paths: users/{userId}/userData/notes/items/{noteId}
        const noteDocRef = doc(firestore, 'users', userId, 'userData', 'notes', 'items', noteId);
    ...
    if (note.videoId) {
      try {
        ...
        try {
          ...
          console.warn('WordStream: Error updating metadata:', metaError);
        }
        
        console.log('WordStream: Note saved successfully to Firestore');
      } catch (firestoreError) {
        console.error('WordStream: Error saving note to Firestore:', firestoreError);
        // If Firestore saving fails, queue for later retry
        queueOperation('saveNote', noteWithId);
      }
    } else {
      // We're offline, queue the operation for later
      console.log('WordStream: Device offline, queueing note save operation');
      queueOperation('saveNote', noteWithId);
    }
    ...
    console.error('WordStream: Error in saveNote:', error);
